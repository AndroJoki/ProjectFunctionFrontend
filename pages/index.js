...
export default function HomePage() {
  ...
  const withdrawAll = async () => {
    if (atm && balance > 0) {
      try {
        let tx = await atm.withdraw(ethers.utils.parseEther(balance.toString()));
        await tx.wait();
        getBalance();
        addToTransactionHistory("Withdraw All", balance);
        setAmount("");
      } catch (error) {
  ...
  const addToTransactionHistory = (type, amount) => {
    const transaction = { type, amount, timestamp: new Date().toLocaleString() };
          ...
          <ul>
            {transactions.map((tx, index) => (
              <li key={index} className="transaction-item">
                {tx.type}: {tx.amount} ETH ({tx.timestamp})
              </li>
            ))}
          </ul>
        </div>
